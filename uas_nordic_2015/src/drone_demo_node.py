...
from autoquad_interface.msg import drone_waypoint, IntStamped, UInt32Stamped
from uas_nordic_2015.msg import drone_pos, distance
...
class ROSNode():
	def __init__(self):
		...
		self.drone2_last_heard = 0.0
		self.drone_timeout = True
		self.drone2_timeout = True
		self.track_timeout = True
		self.track_last_heard = 0.0
		self.track_lat = 0.0
		self.track_lon = 0.0
		self.track_alt = 0.0
		self.track_e = 0.0
		...
		gps2_topic = rospy.get_param("~drone2_gps_sub", "/mavros2/gps/fix")
		...
		distance_topic = rospy.get_param("~distance_pub",'/distance')
		...
		self.drone2_pos_last_heard = 0.0
		self.drone2_pos_timeout = True

		# setup distance topic publisher
		self.distance_pub = rospy.Publisher(distance_topic, distance, queue_size=1)
		...
		rospy.Subscriber(gps2_topic, NavSatFix, self.on_gps2_topic)
		rospy.Subscriber(battery_topic, BatteryStatus, self.on_battery_topic)
	...
	def on_gps2_topic(self, msg):
		self.drone2_last_heard = rospy.get_time()
	...
	def on_gps_topic(self, msg):
		...
		if self.drone_pos.fix > 0:
			...
			self.publish_distance_messages()
	...
	def on_tracker_topic(self, msg):
		if msg.ok == True:
			self.track_last_heard = rospy.get_time()
			self.track_lat = msg.lat
			self.track_lon = msg.lon
			self.track_alt = msg.alt
			self.track_e = msg.e
			self.track_n = msg.n
			self.track_dist = msg.dist
	...
	def publish_distance_messages(self):
		if self.drone_pos_timeout == False and self.track_timeout == False:	
			self.distance.header.stamp = rospy.get_rostime()
			self.distance.hori = sqrt((self.track_e - self.drone_pos.e)**2 + (self.track_n - self.drone_pos.n)**2)
			self.distance.vert = self.drone_pos.alt - self.track_alt
			self.distance_pub.publish (self.distance)
			self.dist_last_update = rospy.get_time()
			
			if self.drone_pos.alt > self.abs_alt_min and self.distance.hori < 35 and abs(self.distance.vert) < 15:
				if self.target_within_threshold_last_update + 5.0 < rospy.get_time():
					self.target_within_threshold_last_update = rospy.get_time()
					if self.distance.vert > 0:
						rospy.logerr (rospy.get_name() + ': Target nearby, going up')
						#self.alt = self.track_alt + self.alt_step
						self.alt += self.alt_step
						if self.alt > self.abs_alt_max:
							self.alt = self.abs_alt_max
					else:
						rospy.logerr (rospy.get_name() + ': Target nearby, going down')
						#self.alt = self.track_alt - self.alt_step
						self.alt -= self.alt_step
						if self.alt < self.abs_alt_min:
							self.alt = self.abs_alt_min
	...
	def updater(self):
		while not rospy.is_shutdown():
			...
			if self.count % 10 == 0:
				...
				# drone2 position timeout
				if self.drone2_pos_last_heard + self.timeout <= rospy.get_time():
					self.drone2_pos_timeout = True
				else:
					self.drone2_pos_timeout = False

				# distance timeout
				if self.dist_last_update + self.timeout <= rospy.get_time():
					self.dist_timeout = True
				else:
					self.dist_timeout = False

				if self.next_status < rospy.get_time():
					self.next_status = rospy.get_time() + 2.0
					if self.dist_timeout == False:
						rospy.logwarn (rospy.get_name() + ': Distance %.0fm %.0fm  Drone alt %.0fm Tracker alt %.0fm dist %.0fm' % (self.distance.hori, self.distance.vert, self.drone_pos.alt, self.track_alt, self.track_dist))
					else:
						s = ''
						if self.drone_timeout == True:
							s = s + 'DRONE_LINK_ERR '
						elif self.drone_pos_timeout == True:
							s = s + 'FIX_ERR '
						else:
							s = s + 'Drone alt: %.0fm ' % (self.drone_pos.alt)
						if self.track_timeout == True:
							s = s + 'TRACK_ERR '
						else:
							s = s + 'Tracker alt %.0fm dist %.0fm ' % (self.track_alt, self.track_dist)
